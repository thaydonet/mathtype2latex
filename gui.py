...
def _strip_image_url_placeholders(text):
    import re

    return re.sub(r'\[Image\s*URL:\s*[^\]]+\]\s*', '', text or '')


def _htmlize_images(text):
    import re

    return re.sub(
        r'!\[\]\(([^)]+)\)',
        r"<img src='\1' class='quiz-math-img' alt='de thi toan' />",
        text or '',
    )


def _split_labeled_options(text, labels, delimiter_pattern):
    import re
    ...
    pattern = re.compile(
        rf'(?<![A-Za-z0-9])([{label_class}])\s*{delimiter_pattern}\s*',
        re.IGNORECASE,
    )
    ...
    for idx, match in enumerate(matches):
        label = match.group(1)
        canonical = next((item for item in labels if item.lower() == label.lower()), label)
        start = match.end()
...
def _repair_mcq_options(opts, content):
    import re
    ...
    split = _split_labeled_options(combined, ['A', 'B', 'C', 'D'], r'[\.\)]')

    for value in opts.values():
        nested = _split_labeled_options(value, ['A', 'B', 'C', 'D'], r'[\.\)]')
        for nested_label, nested_value in nested.items():
            if nested_value:
                split[nested_label] = nested_value

    repaired = {}
    for label in ['A', 'B', 'C', 'D']:
        value = split.get(label) or opts.get(label, '')
        repaired[label] = re.sub(rf'^[{label}{label.lower()}]\s*[\.\)]\s*', '', value).strip()

    order = ['A', 'B', 'C', 'D']
    for idx, label in enumerate(order[:-1]):
        value = repaired.get(label, '')
        tail_pattern = re.compile(
            rf'(?<![A-Za-z0-9])([{ "".join(order[idx + 1:]) }])\s*[\.\)]\s*',
            re.IGNORECASE,
        )
        match = tail_pattern.search(value)
        if match:
            tail_label = match.group(1).upper()
            repaired[label] = value[:match.start()].strip(" \t\r\n")
            repaired[tail_label] = value[match.end():].strip(" \t\r\n")

    return repaired


def _extract_msq_options(content):
    return _split_labeled_options(content or '', ['a', 'b', 'c', 'd'], r'[\)\.]')
...
class ConvertTab:
    ...
    def _do_convert(self, inp, out):
        try:
            ...
            # REMOVED: User requested to disable this automatic step
            # Users can still enable this via the checkbox if needed
            # tmp2 = tempfile.mktemp(suffix=".docx")
            # temps.append(tmp2)
            ...
            if self.export_json_var.get():
                ...
                try:
                    ...
                    # Extract underlined answers from Word document
                    self._log("Trích xuất đáp án gạch dưới từ Word ...")
                    try:
                        from convert_mathtype_to_latex import extract_underlined_answer
                        underlined_answers = extract_underlined_answer(out)
                        self._log(f"Đã tìm thấy {len(underlined_answers)} đáp án gạch dưới.")
                    except Exception as ue:
                        self._log(f"Cảnh báo: Không thể trích xuất đáp án gạch dưới ({ue})")
                        underlined_answers = {}
                    ...
                    if questions:
                        ...
                        for q in questions:
                            ...
                            question_num = None
                            question_match = _re.match(r'^\s*[Cc]âu\s+(\d+)', raw_content)
                            if question_match:
                                question_num = question_match.group(1)

                            # Remove [Image URL: ...] from raw_content
                            raw_content = _strip_image_url_placeholders(raw_content)
                            ...
                            loidgiai_raw = _strip_image_url_placeholders(loidgiai_raw)
                            ...
                            opts = {k: _strip_image_url_placeholders(v) for k, v in opts.items()}
                            ...
                            # OVERRIDE: Use underlined answer from Word document if available
                            if question_num and question_num in underlined_answers:
                                underlined_ans = underlined_answers[question_num]
                                if underlined_ans:
                                    answer = underlined_ans
                                    self._log(f"Câu {question_num}: Sử dụng đáp án gạch dưới = {underlined_ans}")
                            ...
                            if q_type == 'msq':
                                ...
                                if option_start:
                                    content = content[:option_start.start()].strip()

                            content = _htmlize_images(content)

                            explanation_html = loidgiai
                            explanation_html = _htmlize_images(explanation_html)
                            ...
                            if q_type == 'mcq':
                                repaired_opts = _repair_mcq_options(opts, raw_content)
                                opt_a = repaired_opts.get('A', '')
                                opt_b = repaired_opts.get('B', '')
                                opt_c = repaired_opts.get('C', '')
                                opt_d = repaired_opts.get('D', '')
                                
                                mq["option_a"] = _htmlize_images(opt_a)
                                mq["option_b"] = _htmlize_images(opt_b)
                                mq["option_c"] = _htmlize_images(opt_c)
                                mq["option_d"] = _htmlize_images(opt_d)
                            elif q_type == 'msq':
                                full_content = _strip_image_url_placeholders(q.get('content', ''))
                                items = _extract_msq_options(full_content)
                                opt_a = items.get('a', '')
                                opt_b = items.get('b', '')
                                opt_c = items.get('c', '')
                                opt_d = items.get('d', '')
